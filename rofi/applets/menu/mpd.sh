...
current= $(playerctl -p rhythmbox metadata title)
...
case $chosen in
    $previous)
        ...
        playerctl -p rhythmbox previous
        ;;
    $play_pause)
    ...
    $tog_repeat)
        # mpc -q repeat
        playerctl -p rhythmbox loop Track
        ;;
    $tog_random)
        # mpc -q random
        playerctl -p rhythmbox shuffle Toggle
        ;;
esac
